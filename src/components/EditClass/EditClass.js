...
import { Form, Select, Card, InputNumber, Input, Typography, Button, message,  TimePicker, Menu, Dropdown, Radio } from 'antd';
...
export class EditClass extends Component {
    ...
    getClassData = async () => {
        let res = await axios.get(`http://localhost:5000/api/class/${this.props.id}`);
    ...
    render() {
        ...
        const { programData, teacherData, routineFor, subjectName, teacherName, classCode, classGroup, startingPeriod, noOfPeriod, courseCode, link1, weekDay } = this.state;
        console.log( programData, teacherData, routineFor, subjectName, teacherName, classCode, classGroup, startingPeriod, noOfPeriod, courseCode, link1, weekDay );
        return (
            <Card
                className="card"
                style={{ backgroundColor: "#F3F1FF", margin: "12px" }}
            >
                <Title className="input" level={3}>Add/Edit Class</Title>
                <Form {...layout} ref={this.formRef} name="control-ref" onFinish={this.onFinish}>
                    ...
                    <Form.Item label="Starting Period" >
                        <Form.Item name="startingPeriod" rules={[
                            {
                                required: true,
                    ...
                    <Form.Item label="No Of Periods" >
                        <Form.Item name="noOfPeriod" rules={[
                            {
                                required: true,
                    ...
                    <Form.Item {...tailLayout}>
                        <Button type="primary" style={{ backgroundColor: "#141414" }} htmlType="submit">
                            Submit
                        </Button>
                    </Form.Item>
                </Form>
